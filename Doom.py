...
def toggle_mod_skip(map_id):
    ...
    with open(CSV_FILE, "r", encoding="utf-8") as f:
        ...
        if "ModsErlaubt" not in fieldnames and "MOD" not in fieldnames:
            fieldnames.append("ModsErlaubt")

        for row in reader:
            if row.get("ID", "").strip().upper() == search_id:
                found = True
                mod_key = "ModsErlaubt" if "ModsErlaubt" in fieldnames else "MOD"
                current = row.get(mod_key, "0").strip()
                row[mod_key] = "0" if current == "1" else "1"
            rows.append(row)
...
def uninstall_map(map_id):
    ...
    confirm = input(
        f"  {Colors.YELLOW}Tippe 'JA' zum Bestätigen (oder ENTER zum Abbrechen): {Colors.WHITE}"
    ).strip()
...
def get_next_id(game_type):
    ...
    with open(CSV_FILE, "r", encoding="utf-8-sig") as f:
        lines = f.readlines()
        if not lines:
            return "1" if game_type == "DOOM" else f"{prefix}1"
...
def run_installer():
    ...
    if not os.path.exists(INSTALL_DIR):
        ...
        print(
            f"  {Colors.YELLOW}Lege dort deine WADs/ZIPs ab und drücke danach wieder [I].{Colors.WHITE}"
        )
        time.sleep(3)
        return
    ...
    for folder in folders:
        ...
        if not os.path.exists(target_path):
            ...
            print(
                f"  {Colors.GREEN}[+]{Colors.WHITE} Installiert: {Colors.YELLOW}{m_name}{Colors.WHITE} (ID: {new_id})"
            )
            installed_count += 1
        else:
            print(
                f"  {Colors.RED}[!] Übersprungen: {folder} existiert bereits.{Colors.WHITE}"
            )
...
def initial_setup():
    ...
    if not os.path.exists(CSV_FILE):
        ...
        print(
            f"  {Colors.GREEN}[+]{Colors.WHITE} Standard-Datenbank erstellt: {Colors.YELLOW}maps.csv{Colors.WHITE}"
        )
        setup_activity = True
    ...
    if not os.path.exists(UZ):
        ...
        if dl_choice == "j":
            ...
            print(
                f"      Bitte entpacke die {Colors.CYAN}uzdoom.exe{Colors.WHITE} manuell in den Ordner '{Colors.YELLOW}UzDoom{Colors.WHITE}'."
            )
        setup_activity = True

    iwad_path = os.path.join(BASE_DIR, "iwad")
    iwads_found = (
        [f for f in os.listdir(iwad_path) if f.lower().endswith((".wad", ".pk3"))]
    ...
    if not iwads_found:
        print(
            f"  {Colors.RED}[!] WICHTIG: Keine Hauptspiele (IWADs) gefunden!{Colors.WHITE}"
        )
        print(
            f"      Bitte kopiere z.B. {Colors.CYAN}doom.wad{Colors.WHITE} oder {Colors.CYAN}doom2.wad{Colors.WHITE} in den Ordner '{Colors.YELLOW}iwad{Colors.WHITE}'."
        )
        setup_activity = True
    ...
    if not os.path.exists(time_file):
        with open(time_file, "w", encoding="utf-8") as f:
            f.write("0")
        print(
            f"  {Colors.GREEN}[+]{Colors.WHITE} Spielzeit-Tracker erstellt: {Colors.YELLOW}total_time.txt{Colors.WHITE}"
        )
        setup_activity = True
    ...
    if setup_activity:
        ...
        print(
            f" {Colors.YELLOW}Einrichtung abgeschlossen oder Hinweise gefunden.{Colors.WHITE}"
        )
        input(
            f" {Colors.WHITE}Drücke {Colors.GREEN}ENTER{Colors.WHITE}, um den Launcher zu starten... "
        )
...
def search_doomworld():
    ...
    while True:
        ...
        for i in range(current_start, current_end):
            ...
            if not is_installed:
                if clean_filename in [f.lower() for f in installed_files] or any(
                    base_name in f.lower() for f in installed_files
                ):
                    is_installed = True
        ...
        if not sel:
            ...
            if 0 <= idx < total:
                ...
                if not is_already_there:
                    if any(
                        base in f.lower() for f in installed_files
                    ) or os.path.exists(os.path.join(PWAD_DIR, base)):
                        is_already_there = True

                if is_already_there:
                    print(
                        f"  {Colors.CYAN}[INFO] Diese Datei (oder der Ordner '{base}') ist bereits installiert.{Colors.WHITE}"
                    )
                    time.sleep(1.5)
                    continue
...
def launch_game(map_data):
    ...
    if DEBUG_MODE:
        ...
        debug_choice = input(
            f" {Colors.YELLOW}Drücke ENTER zum Starten oder tippe '0' zum Abbrechen: {Colors.WHITE}"
        ).strip()
        if debug_choice == "0":
            print(
                f"          {Colors.RED}Start vom Benutzer abgebrochen.{Colors.WHITE}"
            )
            time.sleep(1.5)
            return
...
def analyze_session(log_file, map_id, mapname, session_seconds):
        ...
        with open(log_file, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                ...
                if any(
                    ...
                    stats["health"] += 1
                if any(x in line_lower for x in ["panzer", "armor", "mega"]):
                    stats["armor"] += 1
                ...
                if any(
                    x in line_lower
                    for x in ["schlüssel", "karte", "keycard", "key", "skull"]
                ):
                    stats["key"] += 1
    ...
    print(
        f"    - Munition: {Colors.YELLOW}{stats['ammo']}{Colors.WHITE} | Schlüssel: {Colors.BLUE}{stats['key']}{Colors.WHITE}"
    )
